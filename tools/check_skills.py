...
def check_python(path: Path, text: str, errors: list[str]) -> int:
    ...
    for start, source in code_blocks(text, "python"):
        ...
        try:
            ...
            errors.append(f"{path.relative_to(ROOT)}:{line}: python block does not parse — {exc.msg}")
    return checked
...
def check_frontmatter(path: Path, text: str, errors: list[str]) -> dict[str, str]:
    ...
    expected = f"harness-{path.parent.name}"
    if fields.get("name") not in (expected, path.parent.name):
        errors.append(
...
def main() -> int:
    ...
    for path in skills:
        ...
        command = COMMANDS / f"{path.parent.name}.md"
        if not command.exists():
            errors.append(f"skills/{path.parent.name}: no matching commands/{path.parent.name}.md")

    for path in sorted(COMMANDS.glob("*.md")):
        total_blocks += check_python(path, path.read_text(encoding="utf-8"), errors)
    ...
    if not args.quiet:
        print(f"OK — {len(skills)} skills, {total_blocks} python blocks compiled, "
